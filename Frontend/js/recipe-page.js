...
const recipeDisplay = async () => {
   ...
   document.getElementById('section-img').innerHTML = `<img src="${recipe.image}" alt = "image de la recette : '${recipe.name}'" class="recipe-photo"/> `;
      ...
      for (let i = 0; i < ingredientDetails.length; i++) {
         if (ingredient.name === ingredientDetails[i].name) {
            document.querySelector('.ingredients').insertAdjacentHTML("afterbegin", `<li class="ingredient-item">
               <img src="${ingredientDetails[i].image}" alt="${ingredient.name}"/>
      ...
      for (let ingredient of step.ingredients) {
         ...
         for (let i = 0; i < ingredientDetails.length; i++) {
            if (ingredient === ingredientDetails[i].name) {
               document.getElementById(`step-ingredients-${index}`).insertAdjacentHTML("beforeend", `<img src="${ingredientDetails[i].image}" alt="${ingredient}" />`);
            };
         };
      };
   });
